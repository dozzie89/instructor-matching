...
def get_file_classes():
    ...
    try:
        ...
        df_classes = df_classes[pd.notnull(df_classes['PRIMARY_INSTR_ID'])]
        df_classes = df_classes[pd.notnull(df_classes['Begin Time'])]
        df_classes = df_classes[pd.notnull(df_classes['End Time'])]
        instructors = set(df_classes['PRIMARY_INSTR_ID'].sort_values().unique())
...
def get_file_faculty():
    ...
    try:
        global df_faculty
        file_name = askopenfilename(filetypes=[("Excel Files", "*.xlsx"), ("All Files", "*.*")])
        df_faculty = pd.read_excel(file_name)
    except Exception as e:
        raise(e)
    #lbl_print["text"] = f"{file_name} loaded successfully."
...
def get_timeline(instructor):
    global df_classes
    all_classes = []
    classes = df_classes[df_classes['PRIMARY_INSTR_ID'] == instructor][['Begin Time', 'End Time', 'M', 'T', 'W', 'R', 'F']].values.tolist()
    if not df_scheduling.empty:
        conflicts = df_scheduling[df_scheduling['CWID'] == instructor][['Begin Time', 'End Time', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday']].values.tolist()
...
def generate_graph(nodes, edges):
    ...
    try:
        ...
        for edge in G.edges():
            u,v = edge
            if df_faculty[df_faculty['Employee ID'] == u]['Department'].values == df_faculty[df_faculty['Employee ID'] == v]['Department'].values:
                same_dep.add(edge)
            else:
                diff_dep.add(edge)
...
def get_name(id):
    global df_faculty
    return (df_faculty[df_faculty['Employee ID'] == id]['Full Name'].values[0])
...
def output_pairs(pairs):
    ...
    df_class_output = df_classes[['Begin Time', 'End Time', 'M', 'T', 'W', 'R', 'F', 'PRIMARY_INSTR_ID', 'Subj', 'Crse', 'Bldg', 'Room']]
    ...
    data = [(
        pair[0],
        get_name(pair[0]),
        df_faculty[df_faculty['Employee ID'] == pair[0]]['Department'].values[0],
        df_faculty[df_faculty['Employee ID'] == pair[0]]['Job Family'].values[0],
        df_faculty[df_faculty['Employee ID'] == pair[0]]['Business Title'].values[0],
        df_faculty[df_faculty['Employee ID'] == pair[0]]['Email - Primary Work or Primary Home'].values[0],
        [df_apply(time) for time in df_class_output[df_class_output['PRIMARY_INSTR_ID'] == pair[0]].values.tolist()],

        pair[1],
        get_name(pair[1]),
        df_faculty[df_faculty['Employee ID'] == pair[1]]['Department'].values[0],
        df_faculty[df_faculty['Employee ID'] == pair[1]]['Job Family'].values[0],
        df_faculty[df_faculty['Employee ID'] == pair[1]]['Business Title'].values[0],
        df_faculty[df_faculty['Employee ID'] == pair[1]]['Email - Primary Work or Primary Home'].values[0],
        [df_apply(time) for time in df_class_output[df_class_output['PRIMARY_INSTR_ID'] == pair[1]].values.tolist()],
    ) for pair in pairs]
...
def info_helper_three():
    ...
    def info_three_manager_helper():
        ...
        for crn in df_classes[df_classes['PRIMARY_INSTR_ID'] == float(inst.get())]['CRN'].sort_values().unique():
        ...
        def info_three_manager_helper_helper():
            global df_classes
            df_classes = df_classes[(df_classes['PRIMARY_INSTR_ID'] != float(inst.get())) | (df_classes['CRN'] == float(inst_class.get()))]
...
def dep_manager():
    ...
    for dep in df_faculty['Department'].sort_values().unique():
    ...
    def dep_manager_helper():
        ...
        df_faculty = df_faculty[df_faculty['Department'].isin(true_set)]
        df_classes = df_classes[df_classes['PRIMARY_INSTR_ID'].isin(df_faculty['Employee ID'].sort_values().unique())]
        instructors = set(df_classes['PRIMARY_INSTR_ID'].sort_values().unique())
...
def download_dep():
    ...
    for department in df_faculty['Department'].sort_values().unique():
